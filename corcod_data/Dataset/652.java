...
public class Main {
  ...
  static class TaskG {
    ...
    public static void dfs2(int i, int parE) {
      if (i == 0) {
        ...
        depth[i] = depth[par] + 1;
        dpPathToRootWithDetours[i] =
          dpPathToRootWithDetours[par] -
          Math.max(0, dp[i] - edges[parE][2] * 2);
    ...
    public void solve(int testNumber, InputReader in, PrintWriter out) {
      ...
      dfs(0, -1);
      dfs2(0, -1);
      GraphLowestCommonAncestor.LCA lca = GraphLowestCommonAncestor.createLCA(
        appearance,
        firstAppearance,
        depth
      );
      ...
      for (int i = 0; i < q; i++) {
        ...
        long res =
          dpPathToRootWithDetours[u] +
          dpPathToRootWithDetours[v] -
          2 *
          dpPathToRootWithDetours[lcaI] +
          dp[lcaI];
        out.println(res);
      }
    }
  ...
  static class MinRangeSparseTable implements ISearchInRange {
    ...
    public MinRangeSparseTable(long[] array, boolean reverseOrdered) {
      ...
      for (int i = 1; i < LCALength; i++) {
        ...
        for (int j = 0; j < sparseTables[i].length; j++) {
          sparseTables[i][j] =
            min(sparseTables[i - 1][j], sparseTables[i - 1][j + jumpSize]);
        }
      }
    }
    ...
    public Pair<Long, Long> queryIndexValueInRange(long l, long r) {
      ...
      int res = min(
        sparseTables[LCAIndex][(int) l],
        sparseTables[LCAIndex][(int) (r - sizeNeeded + 1)]
      );
  ...
  static class GraphLowestCommonAncestor {
    ...
    public static GraphLowestCommonAncestor.LCA createLCA(
      int[] appearances,
      final int[] firstAppearance,
      final int[] depth
    ) {
      return new GraphLowestCommonAncestor.LCA_MinRangeSparseTable(
        appearances,
        firstAppearance,
        depth
      );
    ...
    private static class LCA_MinRangeSparseTable
      ...
      public int getLCA(int a, int b) {
        ...
        return indexToNode[(int) (long) minRangeSparseTable.queryIndexValueInRange(
            l,
            r
          )
            .first];
