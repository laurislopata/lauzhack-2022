...
public class GFG {
  ...
  static void printMinIndexChar(String str, String patt) {
    ...
    for (int i = 0; i < m; i++) if (!hm.containsKey(str.charAt(i))) hm.put(
      str.charAt(i),
      i
    );
    ...
    for (int i = 0; i < n; i++) // if patt[i] is found in 'um', check if
    ...
    if (
      hm.containsKey(patt.charAt(i)) && hm.get(patt.charAt(i)) < minIndex
