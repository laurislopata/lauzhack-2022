...
public class CF903F {
  ...
  static void fill(int[][][][] aa, int a) {
    for (int h0 = 0; h0 <= 4; h0++) for (int h1 = 0; h1 <= 4; h1++) for (
      int h2 = 0;
      h2 <= 4;
      h2++
    ) for (int h3 = 0; h3 <= 4; h3++) aa[h0][h1][h2][h3] = a;
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      c_[0] =
        c_[1] =
          c_[2] = c_[3] = c_[n + 4] = c_[n + 5] = c_[n + 6] = c_[n + 7] = '.';
    }
    ...
    for (int i = 0; i < n + 4; i++) {
      for (int h0 = 0; h0 <= 4; h0++) for (int h1 = 0; h1 <= 4; h1++) for (
        ...
        h2 <= 4;
        h2++
      ) for (int h3 = 0; h3 <= 4; h3++) for (int s = 0; s < 10; s++) {
        hh[0] = h0;
        hh[1] = h1;
        hh[2] = h2;
        hh[3] = h3;
        for (int k = 0; k < 4; k++) {
          int h = ww[s][k];
          if (hh[k] < h) {
            while (h < 4 && cc[k][i + h] == '.') h++;
            hh[k] = h;
          }
        }
        int x = dp[h0][h1][h2][h3] + aa[s];
        if (dp[hh[0]][hh[1]][hh[2]][hh[3]] > x) dp[hh[0]][hh[1]][hh[2]][hh[3]] =
          x;
      }
      fill(dq, INF);
