...
element */
    ...
    for (int i = 1; i < n; i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (arr[j] < arr[i] && las[i][0] < las[j][1] + 1) las[i][0] =
          las[j][1] + 1;
        ...
        if (arr[j] > arr[i] && las[i][1] < las[j][0] + 1) las[i][1] =
          las[j][0] + 1;
      }
...
index i */
      ...
      if (res < Math.max(las[i][0], las[i][1])) res =
        Math.max(las[i][0], las[i][1]);
    }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
      "Length of Longest " + "alternating subsequence is " + zzis(arr, n)
    );
  }
}
