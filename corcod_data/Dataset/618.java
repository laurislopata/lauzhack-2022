...
public class kingrace {
  ...
  public static void main(String[] args) {
    ...
    double x = (double) Math.sqrt(
      Math.abs((xcoin - 1) * (xcoin - 1) + (ycoin - 1) * (ycoin - 1))
    );
    double y = (double) Math.sqrt(
      Math.abs((xcoin - a) * (xcoin - a) + (ycoin - a) * (ycoin - a))
    );
    long c = 0;
    long d = 0;
    if (x > y) {
  ...
  public long Distance(coordinates num) {
    ...
    while ((this.xcoord != num.xcoord || this.ycoord != num.ycoord)) {
      if (num.xcoord - this.xcoord == 1 && num.ycoord == this.ycoord) {
        ...
        num.xcoord - this.xcoord == 0 && num.ycoord - this.ycoord == 1
      ) {
        distance++;
        ...
        num.xcoord - this.xcoord >= 0 && num.ycoord - this.ycoord <= 0
      ) {
        distance++;
