...
public class Solution {
  ...
  static boolean canWinFromOneMove(char[] s, int k) {
    int prefix = 0;
    int n = s.length;
    for (int i = 0; i < n && s[i] == s[0]; i++) prefix++;
    int suffix = 0;
    for (int i = n - 1; i >= 0 && s[i] == s[n - 1]; i--) suffix++;

    return (
      s[0] == s[n - 1] &&
      prefix + suffix + k >= n ||
      Math.max(prefix, suffix) + k >= n
    );
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = n - 2; i >= 0; i--) suff[i] = Math.max(suff[i], suff[i + 1]);
    int max = 0, curr = 0;
    boolean draw = false;
    int ones = 0;
    for (int i = 0; i + k <= n; i++) {
      ...
      if (s[i] == '1') ones++;
      if (i > 0 && s[i] == s[i - 1]) curr++; else curr = 1;
