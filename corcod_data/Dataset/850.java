...
public class codeforces {
  ...
  public static void main(String[] args) {
    ...
    for (int k = 1; k < n - 1; k++) {
      ...
      while (i < j) {
        ...
        if (E[mid] - E[k - 1] <= U) i = mid; else j = mid - 1;
      }
      ...
      if (E[k] - E[i] <= U) max = Math.max(max, T);
  ...
  public static int BinarySearch(long[] a, long k) {
    ...
    if (k < a[0]) return 0; else if (k >= a[n - 1]) return n; else {
      while (j - i > 1) {
        mid = (i + j) / 2;

        if (k >= a[mid]) i = mid; else j = mid;
      }
    }
  ...
  static class CodeX {
    ...
    private static void merge(long A[], long start, long mid, long end) {
      ...
      for (int i = (int) start; i <= end; i++) {
        if (p > mid) Arr[(int) k++] = A[(int) q++]; else if (
          q > end
        ) Arr[(int) k++] = A[(int) p++]; else if (
          A[(int) p] < A[(int) q]
        ) Arr[(int) k++] = A[(int) p++]; else Arr[(int) k++] = A[(int) q++];
      }
      for (int i = 0; i < k; i++) {
        A[(int) start++] = Arr[i];
      }
