...
public class WCS {
  ...
  public static class Vector implements Comparable<Vector> {
    ...
    public int compareTo(Vector v) {
      ...
      if (thisa < 0) thisa += 2 * Math.PI;
      if (va < 0) va += 2 * Math.PI;
      if (thisa < va) return -1;
      if (thisa > va) return 1;
  ...
  public static boolean ok(Vector v1, Vector v2) {
    return (
      v1.add(v2).euclideanNorm() <=
      Math.max(v1.euclideanNorm(), v2.euclideanNorm())
    );
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (vectors.size() == 2) {
      ...
      if (sub.euclideanNorm() <= add.euclideanNorm()) vectors.add(
        sub
    ...
    for (int i = 0; i < n; i++) buffer.append(ans[i] + " ");
