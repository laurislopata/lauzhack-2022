  ...
  static class Solver {
    ...
    public boolean done(int[] sp, int[] par) {
      ...
      for (int i = 1; i < sp.length; i++) {
        if (root != findSet(sp[i], par)) return false;
      }
      return true;
    }
    ...
    public boolean checkPallindrom(String s) {
      ...
      for (int i = 0; i < s.length() / 2; i++) {
        if (ch[i] != ch[s.length() - 1 - i]) return false;
      }
      return true;
    }
    ...
    public static long constructSegment(
      long seg[],
      long arr[],
      int low,
      int high,
    ...
    public static long querySegment(
      long seg[],
      int low,
      int high,
      int qlow,
    ...
    public static int lcs(char[] X, char[] Y, int m, int n) {
      ...
      if (X[m - 1] == Y[n - 1]) return (
        1 + lcs(X, Y, m - 1, n - 1)
      ); else return Integer.max(lcs(X, Y, m, n - 1), lcs(X, Y, m - 1, n));
    }
    ...
    public static long recursion(
      ...
      min =
        Long.min(
          min,
          recursion(start, mid, cnt, a, b) + recursion(mid + 1, end, cnt, a, b)
        );
      // System.out.println("min "+min);
      return min;
    }
