  ...
  public static void main(String ar[]) throws Exception {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (tm.floorKey(l - 1) != null) {
        ...
        if ((v - u) * y < x) {
          ans[i] = ((r - u) * y) % mod;
          if (tm.get(u) > 1) tm.put(u, tm.get(u) - 1); else tm.remove(u);
        } else {
          ans[i] = (x + (r - l) * y) % mod;
        }
      } else ans[i] = (x + (r - l) * y) % mod;
      S = (S + ans[i]) % mod;
      if (tm.containsKey(r)) tm.put(r, 1 + tm.get(r)); else tm.put(r, (long) 1);
    }
...
class The_Comp implements Comparator<B> {
  ...
  public int compare(B b1, B b2) {
    if (b1.x > b2.x) return 1; else if (b1.x == b2.x) {
      if (b1.y > b2.y) return 1; else if (
        b1.y == b2.y
