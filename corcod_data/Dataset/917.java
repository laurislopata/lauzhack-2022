...
class Name implements Comparable<Name> {
  ...
  public int compareTo(Name ob) {
    if (this.x < ob.x) return -1; else if (this.x > ob.x) return 1;
    return 0;
  }
}
...
class Game implements Comparable<Game> {
  ...
  public int compareTo(Game ob) {
    if (this.x < ob.x) return -1; else if (this.x > ob.x) return 1; else {
      if (this.y < ob.y) return -1; else if (
        this.y > ob.y
...
class Node {
  ...
  public boolean equals(Object ob) {
    ...
    Node obj = (Node) ob;
    if (this.a.equals(obj.a) && this.b.equals(obj.b)) return true;
    return false;
  }
...
class SegmentTree {
  ...
  public int[] createSegmentTree(int[] input) {
    ...
    for (int i = 0; i < segmentTree.length; i++) segmentTree[i] =
      Integer.MIN_VALUE;
  ...
  private void constructSegmentTree(
    ...
    segmentTree[pos] =
      Math.max(segmentTree[2 * pos + 1], segmentTree[2 * pos + 2]);
  }

  public int rangeMinimumQuery(
    int[] segmentTree,
    int qlow,
    int qhigh,
    int len
